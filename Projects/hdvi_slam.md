...
**Scale verified independently:** walked perimeter 3.10 m vs. trajectory-integrated length 2.92–2.99 m — 95% agreement. The 5% shortfall is straight-line summing between keyframes cutting the corners. No translation-scale error.
...
Position error is the Euclidean distance between the first and last optimized keyframe pose. Heading error is the rotation about the world vertical axis between those poses, computed as the world-Z component of `R_last · R_0ᵀ` — verified to agree with the reported yaw difference to two decimal places across all five runs.
...
**Profiling redirected the optimization effort away from the intuitive target.** iSAM2 was the obvious suspect and measures at 1–2 ms — "make the backend faster" would have achieved nothing. Within ICP, the registration solver itself is only 27–79 ms; **voxel downsampling and normal estimation account for roughly 80% of ICP time.** That is the next optimization target, and it is not where the search started.

**Adaptive point cap.** ICP cost is monotonic in point count — measured at 13k points → 24 ms, 32k → 79 ms, 51k → 2116 ms, 85k → 2741 ms — and wide-open views produce 7× the voxel count of tight views from the same ~900k raw points. When a downsampled cloud exceeds the cap, the **original** cloud is re-voxelized at `voxel_size × √(n/cap)` rather than randomly thinned, because normals are estimated on this cloud and uniform spacing matters. Worst-case registration dropped from 2914 ms to ~400 ms.
...
The single largest fix in the project, and it had **three compounding causes** — each of which independently capped the rate, so fixing any one alone changed nothing:

1. The SDK's frame aggregate mode was set to `FULL_FRAME_REQUIRE`, which only emits a frameset when *every* stream has a fresh frame — pinning IMU delivery to the depth rate (~10 Hz).
...
A rejected registration was originally discarded entirely, leaving that keyframe's position constrained by the `ImuFactor` alone — the one sensor that physically cannot supply position. That created a hole in the constraint chain, and loop-closure corrections **pooled at the hole**, producing a localized map tear. Rejected edges are now added with sigmas scaled 15×. A fitness-0.35 registration is a poor measurement, not a meaningless one, and a weak constraint beats leaning on the accelerometer.

### ICP rotation sigma was deliberately loosened

Tightened to 0.009 rad, then reverted to 0.03. The tight value claims every keyframe rotation is accurate to half a degree — but ~50–70 chained edges accumulate 12°+ of disagreement with a closure, so the claim is false. **A too-stiff chain cannot absorb a closure smoothly**; the correction pools wherever the graph happens to be softest instead of distributing. Loosening rotation alone moved a circle run from 15.26° final yaw with a torn map to 3.19°.
...
**Depth quality is lighting-dependent, and it constrains rotation.** The Gemini 435Le uses active stereo, which requires visible texture. In a dim room the depth map degrades and ICP is left under-constrained in rotation **while still reporting acceptable fitness** — a silent failure. Measured directly: identical counter-clockwise loops in low light ended at −42°, −46°, and −21° of heading error; the same loop, same code, lights on, ended at −2.8°.
...
**Depth rate is a hardware ceiling.** 1280×800 @ 10 fps or 640×400 @ 20 fps — no 30 or 60 fps mode. At 10 fps and 90°/s of rotation, there is 9° of viewing-angle change between consecutive frames and the intermediate frames do not exist.
...
**The measurement setup was harder to get right than the algorithm.** Early runs scattered 0.42°–5.52° in heading across four configurations — a spread wider than the differences between the configurations, which meant no change could be attributed to anything. Fixing it required a marked physical ground truth and repeat runs on a *fixed* configuration. With those in place, the same system produced 0.75°–1.89°. The scatter was never noise; it was the absence of a controlled protocol.
